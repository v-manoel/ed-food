...
def RemoveReg():
    ...
    #Presupõe-se que tenham havidos saltos para o indice do registro removido
    if WeightTable() == '0%':
...
def WriteLog():
    with open("Graphs/Users/TabelaHash.csv", 'w+', newline ='') as logfile:
        gravar_dados = csv.writer(logfile, delimiter ='\n')
        gravar_dados.writerow([(index,campo) for index,campo in zip(hash_table.keys(),hash_table.values())])
